...
@Component({
  ...
  styleUrls: ['../../agenda/agenda-global/agenda-global.scss']
})
export class PacienteCitasComponent implements OnInit, AfterViewInit {
  ...
  deleteConfirmData: { fecha: string; hora: string } = { fecha: '', hora: '' };
  ...
  ngOnInit() {
    // Los parámetros :id y :representado están en la ruta padre (pacientes/:id/:representado)
    // Como este componente es un child route, debemos usar route.parent.params
    const parentRoute = this.route.parent;
    if (parentRoute) {
      parentRoute.params.subscribe(params => {
        this.cedula = params['id'];
        this.representado = params['representado'];
      });
    }
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => {
      if (this.cedula && this.representado) {
        this.loadCitas();
      }
    }, 100);
  }
  ...
  loadCitas() {
    // Cargar citas del paciente específico
    this.citaService.getCitasPaciente(this.cedula, this.representado).subscribe(citasPaciente => {
      // Cargar todas las citas para mostrar solapamientos
      this.citaService.getTodasLasCitas().subscribe(todasCitas => {
        const calendarApi = this.calendarComponent?.getApi();
        if (calendarApi) {
          // Limpiar eventos existentes
          calendarApi.removeAllEvents();
          
          // Filtrar citas del mismo día/hora para mostrar solapamientos
          const citasAMostrar = todasCitas.filter(c => {
            // Mostrar citas del paciente
            if (c.CedulaPaciente === this.cedula && c.Representado === this.representado) {
              return true;
            }
            // Mostrar citas de otros pacientes en la misma fecha/hora (solapamientos)
            const citaPaciente = citasPaciente.find(cp => 
              cp.FechaCita === c.FechaCita && cp.HoraCita === c.HoraCita
            );
            return citaPaciente !== undefined;
          });
          ...
          citasAMostrar.forEach(c => {
            ...
            const isPacienteCita = c.CedulaPaciente === this.cedula && c.Representado === this.representado;
            
            calendarApi.addEvent({
              id: `${c.FechaCita}|${c.HoraCita}|${c.Silla}`,
              title: isPacienteCita 
                ? `${c.Motivo || 'Cita'} (Silla ${c.Silla})`
                : `Otra cita - ${c.NombrePaciente || 'Paciente'} (Silla ${c.Silla})`,
              start: `${c.FechaCita}T${c.HoraCita}`,
              end: `${c.FechaCita}T${horaFin}`,
              backgroundColor: backgroundColor,
              extendedProps: { ...c, isPacienteCita }
            });
          });
        }
      });
  ...
  handleDateSelect(selectInfo: any) {
    ...
    this.modalMode = 'CREATE';
    this.citaForm = {
      fecha: startStr ? startStr.split('T')[0] : new Date().toISOString().split('T')[0],
  ...
  saveCita() {
    ...
    const nuevaCita: Cita = {
      CodCia: this.selectedCompany || '01',
    ...
    if (this.modalMode === 'CREATE') {
      ...
      if (o.FechaCita !== nuevaCita.FechaCita || o.HoraCita !== nuevaCita.HoraCita || o.Silla !== nuevaCita.Silla) {
  ...
  deleteCita() {
    const o = this.originalEventData;
    this.deleteConfirmData = { fecha: o.FechaCita, hora: o.HoraCita };
